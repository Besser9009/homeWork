...
number_first = random.randint(3, 20)

number_list = []
for i in range(number_first):
    number_list.append(i+1)
number_list.remove(number_list[len(number_list) - 1])

password = []
number_list2 = []
number_list2.extend(number_list)

for num1 in number_list:
    number_list2.remove(num1)
    if num1 == (number_first-1):
        break
    for num2 in number_list2:
        if number_first%(num1+num2) == 0:
            password.append(str(num1)+str(num2))

print(number_first, " - ", *password)
